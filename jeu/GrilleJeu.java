 ...
 * @author Coupé Xavier, Georget Rémy, FOUQUERAY Victor
 * @version 0.1
 */
...
public class GrilleJeu {
    ...
    // Représente la liste des ponts posés
    public List<Pont> listPontPose;
    ...
    */
    private void verifPont(Ile i1, Ile i2){
        //Il faut s'assurer que i1 et i2 sont bien deux voisins réels
        if (erreur == null){ // erreur est la grille qui garde en mémoire dès la première erreur
    ...
    */
    ...
    Pont ajoutePont(String dir1, IleJoueur src, String dir2, IleJoueur dst,Boolean estHypothese){
        if(src.getValPontDir(dir1) == src.getMaxPont()){
            for(Pont p: src.getListePonts(dir1) ){
                if (p.estHypothese() == estHypothese){
                    supprimePont(p);
                    src.supprimePont(dir1,p);
                    
                }
            } 
            for(Pont p: dst.getListePonts(dir2) ){
                if (p.estHypothese() == estHypothese){
                    dst.supprimePont(dir2,p);
                }
            } 
        }else{
            Pont p = new Pont(src,dst, estHypothese);
            src.ajoutePontList(dir1,p);
            dst.ajoutePontList(dir2, p);
            enregistrePont(p);
            return p;
        }
        return null;
    ...
    */
    ...
    /**
     * Encapsule l'appel du undo
    * @param jeu la grille de jeu
    */
    public void undo(GrilleJeu jeu){
        undoRedo.actionUndo(jeu);
    }

    /**
     * Encapsule l'appel du redo
    * @param jeu la grille de jeu
    */
    public void redo(GrilleJeu jeu){
        undoRedo.actionRedo(jeu);
    }

    /**
     * Encapsule l'appel vers l'undoredo
    * @return le premier pont de la pile
    */
    public Pont premierPontPile(){
        return undoRedo.getPremierElement();
    }
    public static void main(String[] args) {
        GrilleJeu testJeu = new GrilleJeu("../niveaux/facile/Facile-5.txt");
        testJeu.afficher_mat_out();

        //Test pour la grille Facile-5.txt

        if (testJeu.pontPossibleEntre((IleJoueur)testJeu.getIleGrilleJoueur(0,0), (IleJoueur)testJeu.getIleGrilleJoueur(0,2))==true){
            System.out.println("Ca fonctionne");
        }else{
            System.out.println("Ca fonctionne pas");
        }

        testJeu.poserPont((IleJoueur)testJeu.getIleGrilleJoueur(0,0), (IleJoueur)testJeu.getIleGrilleJoueur(0,2), true);
        System.out.println(testJeu.listPontPose.size());
        testJeu.poserPont((IleJoueur)testJeu.getIleGrilleJoueur(0,0), (IleJoueur)testJeu.getIleGrilleJoueur(0,2), true);
        System.out.println(testJeu.listPontPose.size());
        testJeu.poserPont((IleJoueur)testJeu.getIleGrilleJoueur(0,0), (IleJoueur)testJeu.getIleGrilleJoueur(0,2), true);
        System.out.println(testJeu.listPontPose.size());
        //System.out.println(testJeu.getIleGrilleJoueur(0,0).getValPontDir(new String("E")));
        //Lancement de test des aides
        //Aide.testTechnique(testJeu);
    }
}